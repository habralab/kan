...
describe("card cover repository", () => {
  ...
  it("collects active and deleted attachment objects owned by one board", async () => {
    const { db, user, board, card } = await seedCard();
    const activeAttachment = await seedAttachment(
      db,
      card.id,
      user.id,
      "active",
    );
    const deletedAttachment = await seedAttachment(
      db,
      card.id,
      user.id,
      "deleted",
    ...
    await db
      .update(cardAttachments)
      .set({ deletedAt: new Date() })
      .where(eq(cardAttachments.id, deletedAttachment.id));

    const [otherBoard] = await db
      .insert(boards)
      .values({
        publicId: "otherboard12",
        name: "Other board",
        slug: "other-board",
      ...
      .returning();
    if (!otherBoard) throw new Error("Failed to seed another board");
    const [otherList] = await db
      .insert(lists)
      .values({
        publicId: "otherlist123",
        name: "Todo",
        index: 0,
      ...
      .returning();
    if (!otherList) throw new Error("Failed to seed another list");
    const [otherCard] = await db
      .insert(cards)
      .values({
        publicId: "othercard456",
        title: "Other card",
        index: 0,
        listId: otherList.id,
        createdBy: user.id,
      ...
      .returning();
    if (!otherCard) throw new Error("Failed to seed another card");
    await seedAttachment(db, otherCard.id, user.id, "other");

    const objects = await cardAttachmentRepo.getStorageObjectsByBoardId(
      db,
      board.id,
    );
    ...
    expect(objects).toEqual(
      expect.arrayContaining([
        {
          publicId: activeAttachment.publicId,
          s3Key: activeAttachment.s3Key,
        },
        {
          publicId: deletedAttachment.publicId,
